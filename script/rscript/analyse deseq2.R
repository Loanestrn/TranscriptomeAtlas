...
counts_data<-read.csv(input,"retine_rpe.csv")
colData<-read.csv(input,"metadata/metadata.csv")
...
res<- results(dds)
res
write.csv(as.data.frame(res), file = "resultats_deseq2.csv")
...
write.csv(as.data.frame(sig_df1), file = "periphery_centre_R.csv")
write.csv(as.data.frame(sig_df2), file = "periphery_RPE_R.csv")
write.csv(as.data.frame(sig_df3), file = "RPE_centre_R.csv")
